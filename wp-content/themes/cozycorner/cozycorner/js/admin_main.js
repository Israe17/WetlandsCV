...
jQuery(function($){
	...
	$('.ts-gallery-box .add-image').on('click', function(event){
        ...
        var _states = [new wp.media.controller.Library({
            filterable: 'uploaded',
            title: cozycorner_admin_texts.select_images,
            multiple: true,
            priority:  20
        })];
        ...
        file_frame = wp.media.frames.file_frame = wp.media({
            states: _states,
            button: {
                text: cozycorner_admin_texts.use_images
            }
        });
	...
	$('.ts-meta-box-field.table .paste-table').on('click', function(e){
		...
		if( typeof localStorage != 'undefined' ){
			...
			if( !data ){
				alert( cozycorner_admin_texts.paste_table_data_error );
				return;
			}
...
jQuery(function($){
	...
	$('.ts-meta-box-field #ts_header_layout').on('change', function(){
		if( $.inArray( $(this).val(), ['v1','v2','v3','v4','v5','v6','v7','v8'] ) != -1 ){
...
jQuery(function($){
	...
	if( add_sidebar_form.length > 0 ){
		...
		$('#ts-add-sidebar').on('click', function(e){
			...
			if( sidebar_name != '' ){
				...
				var data = {
					action: 'cozycorner_add_custom_sidebar'
	...
	if( $('.sidebar-ts-custom-sidebar').length > 0 ){
		...
		$('.sidebar-ts-custom-sidebar .delete-sidebar').on('click', function(){
			...
			var sidebar_nonce = $('#ts_custom_sidebar_nonce').val();
			var ok = confirm( cozycorner_admin_texts.delete_sidebar_confirm );
			if( ok ){
				widget_block.hide();
				var data = {
					action: 'cozycorner_delete_custom_sidebar'
					,sidebar_name: sidebar_name
					,sidebar_nonce: sidebar_nonce
				};
				...
				$.ajax({
					...
					,success : function(response){
						if( response != '' ){
							...
							widget_block.show();
							alert( cozycorner_admin_texts.delete_sidebar_failed );
						}
					}
				});
			}
...
jQuery(function($){
	...
	$( document ).on( 'click', '.ts-product-cat-upload-field .upload-button', function( event ) {
		...
		file_frame = wp.media.frames.downloadable_file = wp.media({
			title: cozycorner_admin_texts.choose_an_image,
			button: {
				text: cozycorner_admin_texts.use_image
			},
			multiple: false
		});
...
jQuery(function($){
	...
	function update_post_edit_view_buttons( sel ){
		...
		if( !sel.siblings('.' + edit_button_class).length ){
			sel.after('<a class="' + edit_button_class + '" href="#" target="_blank" style="display: none">' + cozycorner_admin_texts.edit_post_button_label + '</a>');
		}
		if( !sel.siblings('.' + view_button_class).length ){
			sel.after('<a class="' + view_button_class + '" href="#" target="_blank" style="display: none">' + cozycorner_admin_texts.view_posts_button_label + '</a>');
		}
		
		if( val != 0 && val != '' ){
			sel.siblings('.' + edit_button_class).attr('href', cozycorner_admin_texts.edit_post_url_pattern[post_type].replace('[post_id]', val)).show();
			sel.siblings('.' + view_button_class).hide();
		}
		else{
			sel.siblings('.' + view_button_class).attr('href', cozycorner_admin_texts.view_posts_url_pattern.replace('[post_type]', post_type)).show();
